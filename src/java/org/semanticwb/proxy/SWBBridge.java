...
public class SWBBridge
{
    ...
    public String getHtml(String surl, HttpServletRequest request)
    {
        ...
        try
        {
            ...
            ret = Utils.readInputStream(conex.getInputStream());
        } catch (Exception e)
        {
            e.printStackTrace();
...
//                    }
        ...
        if (con.getHeaderField("Location")!=null)//resc == 302)
        {
            ...
            if(st!=null)ret.setErrorMessage(Utils.readInputStream(st));            
        ...
        } else if (resc == 200)
        {
            ...
            if (usedResponse)
            {
                ...
                for (int x = 1; con.getHeaderField(x) != null; x++)
                {
                    if(!con.getHeaderFieldKey(x).equalsIgnoreCase("Set-Cookie") && !con.getHeaderFieldKey(x).equalsIgnoreCase("Transfer-Encoding"))
                    {
                        //Si es test se parseara por lo que hay que eliminar el tamaño
                        if(con.getContentType()!=null && con.getContentType().startsWith("text"))
                        {
                            //System.out.println("******************** 1:"+x);
                            if(con.getHeaderFieldKey(x).equalsIgnoreCase("Content-Length"))
                            {
                        ...
                        //System.out.println(con.getContentType()+"->"+con.getHeaderFieldKey(x)+"->"+con.getHeaderField(x));
                    }
                }
            }
